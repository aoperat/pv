...
const BareunPelvicApp = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("bareun_saved_items");
    const dates = localStorage.getItem("bareun_completed_dates");
    if (saved) setSavedItems(JSON.parse(saved));
    if (dates) setCompletedDates(JSON.parse(dates));
  }, []);

  useEffect(() => {
    localStorage.setItem("bareun_saved_items", JSON.stringify(savedItems));
  }, [savedItems]);

  useEffect(() => {
    localStorage.setItem(
      "bareun_completed_dates",
    ...
    return (
      <div className="relative w-full max-w-[280px] mx-auto aspect-[3/4] bg-white rounded-3xl overflow-hidden border border-stone-200 shadow-sm">
        ...
        <svg viewBox="0 0 200 300" className="w-full h-full p-4">
          ...
          {/* Clickable Zones with better highlighting */}
          {/* Waist */}
          <g
            onClick={() =>
          ...
          <div className="bg-white rounded-3xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl relative">
            ...
            <div className="p-6 sm:p-8">
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-6 mb-8">
                ...
                {/* Timer moved here for better layout on mobile */}
                <ExerciseTimer />
              </div>
